...
type System = { id: string; name: string; billingRegime: string; property: Property; units: Array<{ unit: Unit }>; tanks: Tank[] };
...
export default function HeatingOilPage() {
  ...
  async function importCsv(event: React.FormEvent) {
    event.preventDefault(); if (!csv.file) return;
    ...
    const data = await response.json();
    if (!response.ok) setStatus(data.error || "CSV-Import fehlgeschlagen"); else { setStatus(`CSV: ${data.imported} importiert, ${data.skippedDuplicates} Duplikate, ${data.invalidRows} ungültig`); await load(); }
  }
  ...
  return <>
    ...
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-8 flex items-end justify-between">
        ...
        {status && <span className={`text-sm ${status === "Gespeichert" ? "text-green-600" : "text-red-600"}`}>{status}</span>}
      </div>
      ...
      <section className="mb-8 rounded-xl border border-zinc-200 bg-white p-6 shadow-sm">
        <h2 className="mb-4 text-lg font-semibold text-zinc-900">Heizsystem anlegen</h2>
        <form className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4" onSubmit={async (event) => { event.preventDefault(); await submit("createSystem", systemForm); }}>
          <label className="text-sm font-medium text-zinc-700">Objekt<select required className={inputClass} value={systemForm.propertyId} onChange={(e) => setSystemForm({ ...systemForm, propertyId: e.target.value, unitIds: [] })}><option value="">Bitte wählen</option>{properties.map((property) => <option key={property.id} value={property.id}>{property.street}, {property.city}</option>)}</select></label>
          ...
          <div><button className={buttonClass}>Heizsystem speichern</button></div>
        </form>
      </section>
      ...
      {systems.length === 0 ? <EmptyState message="Noch kein Heizsystem vorhanden." /> : <div className="mb-8 space-y-5">{systems.map((system) => <section key={system.id} className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm">
        <div className="mb-4 flex items-start justify-between"><div><h2 className="text-lg font-semibold text-zinc-900">{system.name}</h2><p className="text-sm text-zinc-500">{system.property.street}, {system.property.city} · {system.billingRegime}</p></div><span className="rounded-full bg-red-50 px-3 py-1 text-xs font-medium text-red-700">{system.units.length} Wohnungen</span></div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <form onSubmit={importCsv} className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm"><h2 className="mb-4 text-lg font-semibold text-zinc-900">OilFox-CSV importieren</h2><div className="grid gap-4 sm:grid-cols-2"><TankSelect tanks={tanks} value={csv.tankId} onChange={(value) => { const device = oilFox.devices.find((item) => item.tankId === value); setCsv({ ...csv, tankId: value, deviceId: device?.id || "" }); }} /><label className="text-sm font-medium text-zinc-700">CSV-Datei<input required type="file" accept=".csv,text/csv" className={inputClass} onChange={(event) => setCsv({ ...csv, file: event.target.files?.[0] || null })}/></label></div><button className={`${buttonClass} mt-4`}>CSV importieren</button></form>
      </div>
    </main>
  </>;
}
...
function Field({ label, value, onChange, type = "text" }: { label: string; value: string; onChange: (value: string) => void; type?: string }) { return <label className="text-sm font-medium text-zinc-700">{label}<input required type={type} min={type === "number" ? "0" : undefined} step={type === "number" ? "0.01" : undefined} className={inputClass} value={value} onChange={(e) => onChange(e.target.value)} /></label>; }
function Select({ label, value, onChange, options }: { label: string; value: string; onChange: (value: string) => void; options: Array<[string, string]> }) { return <label className="text-sm font-medium text-zinc-700">{label}<select required className={inputClass} value={value} onChange={(e) => onChange(e.target.value)}><option value="">Bitte wählen</option>{options.map(([id, name]) => <option key={id} value={id}>{name}</option>)}</select></label>; }
