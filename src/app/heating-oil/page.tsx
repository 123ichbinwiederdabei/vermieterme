...
type Reading = { id: string; readingDate: string; quantityLiters: string | null; fillLevelPercent?: number | null; batteryLevel?: string | null; validationError?: string | null; distanceCm?: string | null; meteringStatus?: string | null; manuallyInvalidated?: boolean | null; signalStrength?: string | null; source: string; method: string };
...
type Tank = { id: string; name: string; capacityLiters: string | null; deliveryDetectionThresholdLiters: string; location?: string | null; manufacturer?: string | null; model?: string | null; serialNumber?: string | null; tankType?: string | null; material?: string | null; lengthMm?: string | null; widthMm?: string | null; heightMm?: string | null; diameterMm?: string | null; usableVolumeLiters?: string | null; measurementNotes?: string | null; notes?: string | null; deliveries: Delivery[]; stockReadings: Reading[]; inventoryLots: Lot[]; oilFoxDevices: Device[]; deliveryCandidates: Candidate[] };
...
export default function HeatingOilPage() {
  ...
  const [csvPreview, setCsvPreview] = useState<{ imported: number; skippedDuplicates: number; invalidRows: number; unitErrors?: number; detectedColumns?: string[]; warnings: string[]; candidateCount?: number; measuredFrom?: string | null; measuredTo?: string | null } | null>(null);
  ...
  const emptyTankForm = { heatingSystemId: "", name: "Heizöltank", capacityLiters: "", deliveryDetectionThresholdLiters: "200", location: "", manufacturer: "", model: "", serialNumber: "", tankType: "", material: "", lengthMm: "", widthMm: "", heightMm: "", diameterMm: "", usableVolumeLiters: "", measurementNotes: "", notes: "" };
  ...
  return <>
    ...
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      {systems.length === 0 ? <EmptyState message="Noch kein Heizsystem vorhanden." /> : <div className="mb-8 space-y-5">{systems.map((system) => <section key={system.id} className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm">
        ...
        {system.tanks.map((tank) => <div key={tank.id} className="mt-4 rounded-lg border border-zinc-200 p-4"><div className="flex flex-wrap items-start justify-between gap-2"><div><h3 className="font-medium text-zinc-900">{tank.name}</h3><p className="text-xs text-zinc-500">Kapazität {tank.capacityLiters || "—"} L · Liefererkennung ab {tank.deliveryDetectionThresholdLiters} L · {tank.inventoryLots.length} FIFO-Lots</p>{(tank.location || tank.manufacturer || tank.model || tank.serialNumber) && <p className="mt-1 text-xs text-zinc-500">{[tank.location, [tank.manufacturer, tank.model].filter(Boolean).join(" "), tank.serialNumber && `S/N ${tank.serialNumber}`].filter(Boolean).join(" · ")}</p>}</div><div className="flex gap-2">{tank.stockReadings[0] && <div className="text-right"><p className="text-xl font-semibold text-zinc-900">{tank.stockReadings[0].quantityLiters ?? "—"} L</p><p className="text-xs text-zinc-500">{tank.stockReadings[0].fillLevelPercent ?? "—"}% · Batterie {tank.stockReadings[0].batteryLevel ?? "—"}</p></div>}<button type="button" className="rounded-lg border border-zinc-300 px-3 py-1.5 text-xs font-medium text-zinc-700 hover:bg-zinc-50" onClick={() => { setEditingTankId(tank.id); setTankForm({ heatingSystemId: system.id, name: tank.name, capacityLiters: tank.capacityLiters || "", deliveryDetectionThresholdLiters: tank.deliveryDetectionThresholdLiters, location: tank.location || "", manufacturer: tank.manufacturer || "", model: tank.model || "", serialNumber: tank.serialNumber || "", tankType: tank.tankType || "", material: tank.material || "", lengthMm: tank.lengthMm || "", widthMm: tank.widthMm || "", heightMm: tank.heightMm || "", diameterMm: tank.diameterMm || "", usableVolumeLiters: tank.usableVolumeLiters || "", measurementNotes: tank.measurementNotes || "", notes: tank.notes || "" }); document.getElementById("tank-form")?.scrollIntoView({ behavior: "smooth" }); }}>Bearbeiten</button></div></div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div id="tank-form"><DataForm title={editingTankId ? "Heizöltank bearbeiten" : "Tank anlegen"} onSubmit={async () => { const saved = await submit(editingTankId ? "updateTank" : "createTank", { ...tankForm, ...(editingTankId ? { id: editingTankId } : {}) }); if (saved) { setEditingTankId(null); setTankForm(emptyTankForm); } return saved; }}><Select label="Heizsystem" value={tankForm.heatingSystemId} onChange={(value) => setTankForm({ ...tankForm, heatingSystemId: value })} options={systems.map((system) => [system.id, system.name])} /><Field label="Bezeichnung" value={tankForm.name} onChange={(value) => setTankForm({ ...tankForm, name: value })} /><Field label="Kapazität (L)" type="number" value={tankForm.capacityLiters} onChange={(value) => setTankForm({ ...tankForm, capacityLiters: value })} /><Field label="Liefererkennung ab (L)" type="number" value={tankForm.deliveryDetectionThresholdLiters} onChange={(value) => setTankForm({ ...tankForm, deliveryDetectionThresholdLiters: value })} /><Field label="Standort" value={tankForm.location} onChange={(value) => setTankForm({ ...tankForm, location: value })} /><Field label="Hersteller" value={tankForm.manufacturer} onChange={(value) => setTankForm({ ...tankForm, manufacturer: value })} /><Field label="Modell" value={tankForm.model} onChange={(value) => setTankForm({ ...tankForm, model: value })} /><Field label="Seriennummer" value={tankForm.serialNumber} onChange={(value) => setTankForm({ ...tankForm, serialNumber: value })} /><Field label="Tanktyp" value={tankForm.tankType} onChange={(value) => setTankForm({ ...tankForm, tankType: value })} /><Field label="Material" value={tankForm.material} onChange={(value) => setTankForm({ ...tankForm, material: value })} /><Field label="Länge (mm)" type="number" value={tankForm.lengthMm} onChange={(value) => setTankForm({ ...tankForm, lengthMm: value })} /><Field label="Breite (mm)" type="number" value={tankForm.widthMm} onChange={(value) => setTankForm({ ...tankForm, widthMm: value })} /><Field label="Höhe (mm)" type="number" value={tankForm.heightMm} onChange={(value) => setTankForm({ ...tankForm, heightMm: value })} /><Field label="Durchmesser (mm)" type="number" value={tankForm.diameterMm} onChange={(value) => setTankForm({ ...tankForm, diameterMm: value })} /><Field label="Nutzvolumen (L)" type="number" value={tankForm.usableVolumeLiters} onChange={(value) => setTankForm({ ...tankForm, usableVolumeLiters: value })} /><Field label="Mess-/Kalibrierhinweise" value={tankForm.measurementNotes} onChange={(value) => setTankForm({ ...tankForm, measurementNotes: value })} /><Field label="Notiz" value={tankForm.notes} onChange={(value) => setTankForm({ ...tankForm, notes: value })} /></DataForm></div>
...
function Field({ label, value, onChange, type = "text", required = false }: { label: string; value: string; onChange: (value: string) => void; type?: string; required?: boolean }) { return <label className="flex flex-col gap-1 text-sm font-medium text-zinc-700"><span>{label}</span><input required={required} type={type} min={type === "number" ? "0" : undefined} step={type === "number" ? "0.01" : undefined} className={inputClass} value={value} onChange={(e) => onChange(e.target.value)} /></label>; }
