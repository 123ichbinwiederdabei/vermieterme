...
type Tariff = { id: string; validFrom: string; validTo?: string | null; priceCentsPerKwh: number; monthlyBasePriceCents: string; name?: string | null };
...
export default function ElectricityPage() {
  ...
  const [tariff, setTariff] = useState({ contractId: "", validFrom: "", validTo: "", name: "", priceEuroPerKwh: "", monthlyBasePriceEuro: "" });
  ...
  return <><Nav /><main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
    <div className="mb-8 flex items-end justify-between"><div><h1 className="text-2xl font-bold text-zinc-900">Strom</h1><p className="mt-1 text-sm text-zinc-500">Verträge, Tarife, Zähler und tatsächlicher Verbrauch</p></div>{message && <p className={message === "Gespeichert" ? "text-sm text-green-600" : "text-sm text-red-600"}>{message}</p>}</div>
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      <Form title="Tarifperiode" onSubmit={() => submit("createTariff", { ...tariff, priceCentsPerKwh: euroToCents(tariff.priceEuroPerKwh), monthlyBasePriceCents: euroToCents(tariff.monthlyBasePriceEuro) })}><Select label="Vertrag" value={tariff.contractId} set={(value) => setTariff({ ...tariff, contractId: value })} options={contracts.map((row) => [row.id, row.provider])}/><Field label="Tarifname" value={tariff.name} set={(value) => setTariff({ ...tariff, name: value })} required={false}/><Field label="Gültig ab" type="date" value={tariff.validFrom} set={(value) => setTariff({ ...tariff, validFrom: value })}/><Field label="Gültig bis" type="date" value={tariff.validTo} set={(value) => setTariff({ ...tariff, validTo: value })} required={false}/><Field label="Arbeitspreis (€/kWh)" type="number" value={tariff.priceEuroPerKwh} set={(value) => setTariff({ ...tariff, priceEuroPerKwh: value })}/><Field label="Grundpreis (€/Monat)" type="number" value={tariff.monthlyBasePriceEuro} set={(value) => setTariff({ ...tariff, monthlyBasePriceEuro: value })}/></Form>
...
function Field({ label, value, set, type = "text", required = true }: { label: string; value: string; set: (value: string) => void; type?: string; required?: boolean }) { return <label className="text-sm font-medium text-zinc-700">{label}<input className={fieldClass} type={type} required={required} min={type === "number" ? "0" : undefined} step={type === "number" ? "0.01" : undefined} value={value} onChange={(event) => set(event.target.value)}/></label>; }
function Select({ label, value, set, options }: { label: string; value: string; set: (value: string) => void; options: string[][] }) { return <label className="text-sm font-medium text-zinc-700">{label}<select className={fieldClass} required value={value} onChange={(event) => set(event.target.value)}><option value="">Bitte wählen</option>{options.map(([id, name]) => <option key={id} value={id}>{name}</option>)}</select></label>; }
