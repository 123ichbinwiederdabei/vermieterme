...
import { centsToEuro, euroToCents, euroToMicroEuros, microEurosPerKwhToEuro, microEurosPerKwhToInput } from "@/lib/money";

type Unit = { id: string; name: string };
type Property = { id: string; street: string; city: string; units?: Unit[] };
type Tariff = { id: string; validFrom: string; validTo?: string | null; billingValidFrom?: string | null; billingValidTo?: string | null; billingEffectiveReason?: string | null; priceMicroEuroPerKwh: string; monthlyBasePriceCents: string; name?: string | null };
...
const meterRoleLabel: Record<string, string> = { UNIT_CONSUMPTION: "Wohnungsverbrauch", COMMON_ELECTRICITY: "Allgemeinstrom", INFORMATIONAL_TOTAL: "Summenzähler (informativ)", SMALL_WASTEWATER_ELECTRICITY: "Kleinkläranlage – Anlagenstrom" };
...
export default function ElectricityPage() {
  ...
  const [editingPriceEuroPerKwh, setEditingPriceEuroPerKwh] = useState("");
  ...
  return <><Nav /><main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      <Form title="Tarifperiode" onSubmit={() => submit("createTariff", { ...tariff, priceMicroEuroPerKwh: euroToMicroEuros(tariff.priceEuroPerKwh), monthlyBasePriceCents: euroToCents(tariff.monthlyBasePriceEuro) })}><Select label="Vertrag" value={tariff.contractId} set={(value) => setTariff({ ...tariff, contractId: value })} options={contracts.map((row) => [row.id, row.provider])}/><Field label="Tarifname" value={tariff.name} set={(value) => setTariff({ ...tariff, name: value })} required={false}/><Field label="Vertrag gültig ab" type="date" value={tariff.validFrom} set={(value) => setTariff({ ...tariff, validFrom: value })}/><Field label="Vertrag gültig bis" type="date" value={tariff.validTo} set={(value) => setTariff({ ...tariff, validTo: value })} required={false}/><Field label="Abrechnung wirksam ab" type="date" value={tariff.billingValidFrom} set={(value) => setTariff({ ...tariff, billingValidFrom: value })} required={false}/><Field label="Abrechnung wirksam bis" type="date" value={tariff.billingValidTo} set={(value) => setTariff({ ...tariff, billingValidTo: value })} required={false}/><Field label="Grund der Ersatzregel" value={tariff.billingEffectiveReason} set={(value) => setTariff({ ...tariff, billingEffectiveReason: value })} required={false}/><Field label="Arbeitspreis (€/kWh, bis 6 Nachkommastellen)" type="number" value={tariff.priceEuroPerKwh} set={(value) => setTariff({ ...tariff, priceEuroPerKwh: value })}/><Field label="Grundpreis (€/Monat)" type="number" value={tariff.monthlyBasePriceEuro} set={(value) => setTariff({ ...tariff, monthlyBasePriceEuro: value })}/></Form>
      {editingTariff && <Form title="Tarif und Abrechnungswirksamkeit ändern" onSubmit={async () => { await submit("updateTariff", { id: editingTariff.id, priceMicroEuroPerKwh: euroToMicroEuros(editingPriceEuroPerKwh), billingValidFrom: editingTariff.billingValidFrom || null, billingValidTo: editingTariff.billingValidTo || null, billingEffectiveReason: editingTariff.billingEffectiveReason || null }); setEditingTariff(null); }}><Field label="Vertrag gültig ab" value={new Date(editingTariff.validFrom).toLocaleDateString("de-DE")} set={() => {}} required={false}/><Field label="Arbeitspreis (€/kWh, bis 6 Nachkommastellen)" type="number" value={editingPriceEuroPerKwh} set={setEditingPriceEuroPerKwh}/><Field label="Abrechnung wirksam ab" type="date" value={editingTariff.billingValidFrom?.slice(0, 10) || ""} set={(value) => setEditingTariff({ ...editingTariff, billingValidFrom: value })} required={false}/><Field label="Abrechnung wirksam bis" type="date" value={editingTariff.billingValidTo?.slice(0, 10) || ""} set={(value) => setEditingTariff({ ...editingTariff, billingValidTo: value })} required={false}/><Field label="Dokumentierte Ersatzregel" value={editingTariff.billingEffectiveReason || ""} set={(value) => setEditingTariff({ ...editingTariff, billingEffectiveReason: value })} required={false}/></Form>}
      <Form title="Stromzähler" onSubmit={() => submit("createMeter", meter)}>
        <Select label="Vertrag" value={meter.contractId} set={(value) => setMeter({ ...meter, contractId: value, unitId: "" })} options={contracts.map((row) => [row.id, row.provider])}/>
        <Select label="Rolle" value={meter.role} set={(value) => setMeter({ ...meter, role: value, unitId: value === "UNIT_CONSUMPTION" ? meter.unitId : "" })} options={[["UNIT_CONSUMPTION", "Wohnungsverbrauch"], ["COMMON_ELECTRICITY", "Allgemeinstrom"], ["SMALL_WASTEWATER_ELECTRICITY", "Kleinkläranlage – Anlagenstrom"], ["INFORMATIONAL_TOTAL", "Summenzähler (informativ)"]]}/>
        {meter.role === "UNIT_CONSUMPTION" && <Select label="Wohnung" value={meter.unitId} set={(value) => setMeter({ ...meter, unitId: value })} options={(selectedContract?.property.units || properties.find((row) => row.id === selectedContract?.property.id)?.units || []).map((row) => [row.id, row.name])}/>}
        {meter.role === "SMALL_WASTEWATER_ELECTRICITY" && <p className="sm:col-span-2 text-sm text-zinc-500">Der Anlagenzähler wird ausschließlich der Kleinkläranlage zugeordnet. Es wird nur der gemessene Arbeitspreis abgerechnet; kein Vertragsgrundpreis.</p>}
        <Field label="Zählernummer" value={meter.meterNumber} set={(value) => setMeter({ ...meter, meterNumber: value })}/>
        <Field label="Gültig ab" type="date" value={meter.validFrom} set={(value) => setMeter({ ...meter, validFrom: value })}/>
      </Form>
...
function Field({ label, value, set, type = "text", required = true }: { label: string; value: string; set: (value: string) => void; type?: string; required?: boolean }) { const isElectricityRate = label.startsWith("Arbeitspreis"); return <label className="flex flex-col gap-1 text-sm font-medium text-zinc-700"><span>{label}</span><input className={fieldClass} type={type} required={required} min={type === "number" ? "0" : undefined} step={type === "number" ? (isElectricityRate ? "0.000001" : "0.01") : undefined} value={value} onChange={(event) => set(event.target.value)}/></label>; }
